...
for l in open(SESSION):
    ...
    if t=='shot_detected':
        sd[sn]=dict(ts=o['ts'][11:19],club=o.get('club'),imp=o.get('impact_timestamp'),impk=o.get('impact_timestamp_kld7'),lv=o.get('launch_angle_vertical'))
    elif t=='rolling_buffer_capture':
        rawbs[sn]=o.get('ball_speed_mph')         # RAW OPS, pre-cosine
...
def run(dist, off):
    return [(s['ts'], angle(fr,bs,s['imp'],s['impk'],dist,off), s['lv'], TM[s['ts']]) for s,bs,fr in work]
def stats(per):
    e=np.abs(np.array([v-tm for ts,v,lg,tm in per if v is not None]))
    return e.mean(),np.percentile(e,50),np.percentile(e,90),len(e)
print('=== VALIDATION: tracker + RAW-OPS bs (rolling_buffer) (5.0,off1.5) vs LOGGED ===')
...
for ts,v,lg,tm in run(5.0,1.5):
    ...
    flag=' OK' if d<0.5 else (' ~' if d<1.5 else ' XX')
    print('  %8s %s %6.1f %5.1f   %4.1f%s'%(ts,('%6.1f'%v if v is not None else '   nan'),(lg or 0),tm,d,flag))
