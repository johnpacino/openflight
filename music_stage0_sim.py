...
def est_music_fbss(x, noise_var):
    ...
    # Physical pick: fit complex source amplitudes at the candidate angles on
    # the full array and keep the stronger source. Spurious pseudospectrum
    # peaks fit ~zero amplitude; in deep-fade frames the image wins, which the
    # robust trajectory fit then rejects as a down-outlier.
    ths = GRID[top]
